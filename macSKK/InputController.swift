            ...
            if let self {
                ...
                if let candidates {
                    ...
                    if let page = candidates.page {
                        ...
                        let windowLevel = NSWindow.Level(rawValue: Int(textInput.windowLevel() + 1))
                        Global.candidatesPanel.show(windowLevel: windowLevel)
                    } else {
            ...
            if let self {
                if let completion = Global.dictionary.findCompletion(prefix: yomi) {
                    ...
                    if cursorPosition != .zero {
                        Global.completionPanel.show(at: cursorPosition, windowLevel: windowLevel)
                    }
                } else {
