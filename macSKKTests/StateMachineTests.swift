...
final class StateMachineTests: XCTestCase {
    ...
    func testHandleNormalAbbrev() {
        ...
        stateMachine.inputMethodEvent.collect(12).sink { events in
            ...
            XCTAssertEqual(events[4], .markedText(MarkedText(text: "▽alL", cursor: nil)))
            XCTAssertEqual(events[5], .markedText(MarkedText(text: "▽alLq", cursor: nil)))
            XCTAssertEqual(events[6], .markedText(MarkedText(text: "▽alLqQ", cursor: nil)))
            XCTAssertEqual(events[7], .markedText(MarkedText(text: "▽alLqQ_", cursor: nil)))
            XCTAssertEqual(events[8], .markedText(MarkedText(text: "▽alLqQ_<", cursor: nil)))
            XCTAssertEqual(events[9], .markedText(MarkedText(text: "▽alLqQ_<>", cursor: nil)))
            XCTAssertEqual(events[10], .markedText(MarkedText(text: "▽alLqQ_<>?", cursor: nil)))
            XCTAssertEqual(events[11], .markedText(MarkedText(text: "▽alLqQ_<>?A", cursor: nil)))
            expectation.fulfill()
        }.store(in: &cancellables)
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "/")))
        XCTAssertTrue(stateMachine.handle(printableKeyEventAction(character: "a")))
