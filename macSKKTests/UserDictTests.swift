...
final class UserDictTests: XCTestCase {
    ...
    func testReferMergeAnnotation() async throws {
        let privateMode = CurrentValueSubject<Bool, Never>(false)
        ...
        let words = try await userDict.referDicts("い").map({ $0.word })
        XCTAssertEqual(words, ["胃", "伊", "意"])
        let annotationDictIds = (try await userDict.referDicts("い")).map({ $0.annotations.map({ $0.dictId }) })
        XCTAssertEqual(annotationDictIds, [["dict1", "dict2"], [], []])
    }
