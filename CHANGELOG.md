...
- ひらがなと直接入力を切り替えるキーバインドを追加 (#511)
- テストのOptional展開をXCTUnwrapに置き換える (#513)
- Xcode 27でのコンパイルエラーおよび警告を修正 (#514)
...
- ">" 単体の変換で空見出しを辞書登録しないようにする (#502) by @delphinus
- UserDictをバックグラウンドスレッドから辞書検索できるようにする (#500)

## v2.18.2 (2026-07-19)

- 補完候補検索処理をCompletionPresenterへ切り出しユニットテストを追加 (#499)
- ACT09 (ACTの改良版) ローマ字入力ルールを追加 (#498)
...
- ドキュメントサイトを作成 (#482)
  - https://mtgto.github.io/macSKK/
  - vite, esbuildの脆弱性解消のためにVitePressをv2アルファ版にアップデート (#483)
- 補完候補検索時のskkserv問い合わせ回数とcandidate総数に上限を設ける (#485)
- 送り仮名入力中に遅延した補完候補検索結果でパネルが表示されるバグを修正 (#487)

## v2.17.0 (2026-06-13)

- メニューに直近の辞書登録をキャンセルする機能を追加 (#472) by @tekezo
- キーボード操作での削除時に入力メニューに残る問題などを修正 (#475)
- 送り仮名エントリの削除を送り仮名が一致する候補のみに限定する (#476)
- 変換候補・補完候補の数を設定可能にする (#474)
- 補完候補の検索が該当読み数に対してO(n^2)になっているのを修正 (#478) by @delphinus
- 読み入力から候補選択に切り替わるまでの時間を設定化 (#479)
- 補完候補確定キーの選択範囲をdisplayCandidateCountに制限 (#481)

## v2.16.0 (2026-05-30)

- 単語登録画面でタブキーで入力文字列を補完する設定を追加 (#79)
- GitHub Actions環境をXcode 26.5に変更 (before 26.4) (#468)
- 設定画面のフォント名をローカライズして表示 (#467)
- 補完候補から直接キーで確定できる (#469)
- 背景色設定と横表示でカスタムフォントが反映されない問題を修正 (#470)

## v2.15.0 (2026-05-17)

- 単語登録中に空文字列で前候補かバックスペースキーを押したときに候補選択に戻る設定を追加 (#463) by @tekezo
- 変換候補選択中のバックスペースに「前候補に戻す」を追加 (#464) by @tekezo
- アプリケーション互換性設定に「空のときには▽▼を表示」を追加 (#462)
...
- FileDict/UserDictのSwift 6 Concurrency対応 (#452)

## v2.12.1 (2026-04-11)

- すでに送り仮名の入力中に`<okuri>`ルールによる送り仮名の上書きをしないよう修正 (#453)
...
- 入力モードごとのテキスト色・背景色をカスタマイズできるようにする (#446)
- SKKServの接続エラーが連続したとき自動で無効化する (#448)
- GitHub Actions環境をXcode 26.3に変更 (#447)

## v2.10.4 (2026-03-08)

- 辞書ファイルの保存を一時ファイル経由のアトミック書き込みに変更 (#445)

## v2.10.3 (2026-03-01)

- Abbrevモード入力中にCtrl-Jで確定したときは元のモードに戻す (#444)

## v2.10.2 (2026-02-28)

- デフォルトのローマ字かな変換ルールを使用するかのディレクティブを追加 (#440)

## v2.10.1 (2026-02-23)

- Ctrl-Jで確定するときひらがなモードに変わってしまうのを修正 (#437)
- 起動時にデフォルトのローマ字かな変換ルールを最初に読み込んでなかったのを修正 (#438)

## v2.10.0 (2026-02-08)

- 複数のローマ字かな変換ルールを設定から切り替え可能にする (#432)
- ローマ字かな変換ルールファイルの設定キーを間違って使っている場所を修正 (#433)

## v2.9.1 (2026-01-19)

- 変換候補の横方向表示で現在選択されている候補の色が見えないバグを修正 (#429)

## v2.9.0 (2026-01-19)

- 補完・変換候補ウィンドウのフォント・背景色を設定可能にする (#424)
- 候補選択時に/入力で現在の候補の確定後にAbbrevモードに戻れないバグを修正 (#426)

## v2.8.0 (2025-12-21)

- GitHub Actions環境をXcode 26.2に変更 (before 26.1) (#418)
- 設定から最新バージョンの取得に失敗した場合にログを出す (#417)
- カタカナで確定した単語を辞書に保存するかの設定項目を追加 (#421)

## v2.7.0 (2025-11-30)

- 直接入力モードからAbbrevモードに入るキーバインドを追加 (#412)
- GitHub Actions環境をXcode 26.1に変更 (before 26.0) (#413)

## v2.6.0 (2025-11-09)

- SKKServに補完候補を問い合わせする設定を追加 (#407)
- SKKServの補完候補の応答で重複要素や入力済みの読みと同じ候補は除外する (#408)

## v2.5.0 (2025-10-19)

- 読みの補完候補があるときShift-Spaceで先頭候補で変換開始する (#392)
- CandidatesViewModel.minWidthの計算にcandidatesFontSizeを使う (#404) by @y-yu
- ピリオドキーで補完候補の先頭を確定する設定を追加 (#403)

## v2.4.2 (2025-09-23)

- 重複した変換候補をもつエントリは読み込み時に重複要素を除去する (#396)
- 読みにスペースを含むエントリを禁止する (#397)
- 補完候補ビューのサイズを計算して余白が表示されないように修正 (#398)
- 読み入力中じゃないのに補完候補が表示されたりするバグを修正 (#399)
- 補完候補で読みを表示するときの余白を消していたのを修正。高さ計算を修正 (#400)
- GitHub Actions環境をmacOS 26 & Xcode 26.0に変更 (before macOS 15 & 16.4) (#401)

## v2.4.1 (2025-09-13)

- 補完候補の検索で読み込み専用辞書のみ二分探索を行う (#393)
- 補完候補表示位置のためのカーソル座標をキー入力時に取得 (#394)
