...
| backToSelectingFromRegistering | Boolean | 単語登録中に空文字列で前候補キーもしくはバックスペースキーで候補選択に戻るか |
| showsMarkedTextMarker       | Boolean    | ▽と▼の表示                              |
| yomiCompletionByTabInRegistering | Boolean | 単語登録中にタブキーで読みを補完するか |
| displayCandidateCount       | Number     | 変換候補パネルに一度に表示する変換候補の数 |
| completionConfirmationTimeLimit | Number | 読み入力から候補選択に切り替わるまでの時間 (ミリ秒) |
